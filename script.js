...
function loadData() {
  const saved = localStorage.getItem("miTurnoV15");
  if (saved) return JSON.parse(saved);

  localStorage.setItem("miTurnoV15", JSON.stringify(appDataDefault));
  return structuredClone(appDataDefault);
}

function saveData() {
  localStorage.setItem("miTurnoV15", JSON.stringify(appData));
}
...
$("reserveForm").onsubmit = (e) => {
  ...
  $("statusText").textContent = "Se abrió WhatsApp con el mensaje listo. Tocá enviar para que el complejo reciba tu solicitud.";
...
function renderPendingReservations() {
  const pending = appData.reservations.filter(r =>
    r.complexName === appData.complex.name && r.status === "pendiente"
  );
...
$("newSearchBtn").onclick = () => show("home");
...
$("ownerLoginForm").onsubmit = (e) => {
  ...
  if ($("ownerUser").value === appData.owner.user && $("ownerPass").value === appData.owner.pass) {
    renderPendingReservations();
    renderOwnerConfig();
    renderOwnerCourts();
    show("ownerPanel");
